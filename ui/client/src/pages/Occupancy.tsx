...
export default function Occupancy() {
  ...
  return <AppShell>
    <SectionIntro eyebrow="OCCUPANCY / FLOOR SIGNAL" title="See the office in motion." description="Current occupancy and floor distribution, read directly from access events." action={<div className="date-control"><label htmlFor="occupancy-date">Report date</label><input id="occupancy-date" type="date" value={date} onChange={(event) => setDate(event.target.value)} /><button className="icon-button" onClick={() => void load()} aria-label="Refresh occupancy"><RefreshCw size={16} /></button></div>} />
    {loading ? <LoadingState label="Loading occupancy" /> : error ? occupancySkeleton : !data ? <EmptyState title="No occupancy signal" description="The backend did not return occupancy for this date." /> : <>
      <div className="metric-grid occupancy-metrics"><MetricCard label="Inside office" value={data.total_occupancy ?? 0} meta={`as of ${data.date || date}`} tone="cyan" icon={<Activity size={15} />} /><MetricCard label="Floors reporting" value={Object.keys(floorBreakdown).length} meta="active floor signals" tone="ink" /><MetricCard label="Peak employees" value={data.peak_count ?? "—"} meta={data.peak_time ? formatPeakTime(data.peak_time) : "Peak not exposed by endpoint"} tone="amber" icon={<RadioTower size={15} />} /></div>
      <SectionRule label="FLOOR DISTRIBUTION" />
      <section className="data-panel wide-panel"><div className="panel-heading"><div><span className="eyebrow"><span className="eyebrow-mark">01</span>LIVE FLOOR READ</span><h2>Where people are now</h2></div><span className="live-pulse"><span /> Source: employee access</span></div>{Object.keys(floorBreakdown).length ? <div className="large-floor-list">{Object.entries(floorBreakdown).map(([floor, value]) => <div className="large-floor-row" key={floor}><div className="large-floor-label"><span>{floor}</span><strong>{value}<small> employees</small></strong></div><div className="large-bar-track"><div className="large-bar-fill" style={{ width: `${(value / floorMax) * 100}%` }} /></div><span className="floor-share">{data.total_occupancy ? Math.round((value / data.total_occupancy) * 100) : 0}%</span></div>)}</div> : <EmptyState title="No floor records" description="There are no check-in records for this date." />}<div className="panel-footnote"><span className="registration-mark">+</span><span>Peak time: <strong>{data.peak_time ? formatPeakTime(data.peak_time) : "Not returned by /occupancy"}</strong></span><a href="/dashboard">Back to dashboard <ArrowUpRight size={14} /></a></div></section>
    </>}
  </AppShell>;
}
